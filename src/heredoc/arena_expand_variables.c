...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   arena_expand_variables.c                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: anpollan <anpollan@student.hive.fi>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/06 13:54:02 by anpollan          #+#    #+#             */
/*   Updated: 2025/10/15 10:46:12 by anpollan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*arena_expand_variables(char *input, t_shell *shell)
{
	...
	if (!inputv)
		return (NULL);
	expanded = arena_strdup("", shell->command_arena);
...
static char	*handle_word(char **inputv, char *expanded, t_shell *shell)
{
	...
	while (inputv[i])
	{
		...
		expanded = arena_strdup(temp, shell->command_arena);
		free(temp);
		i++;
	}
	return (expanded);
